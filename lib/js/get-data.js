...
function plot_observation(observation_data)
{
    ...
    svg_plot.append("rect")
            ...
            .attr("width", width)
            .attr("height", yScale(observation_data.normal_range.max) 
            ...
            .enter().append("circle")
                    ...
                    .on("mouseover", function(event, d){
                      tooltip_div.transition()
                                  .duration(200)
                                  .style("opacity", 0.9);
                      tooltip_div.html(d.time.year() + "<br/>" + d.value + " " + d.units)
                          .style("left", xScale(data.time) + "px")
                          .style("top", (yScale(data.value) - 40) + "px");
                        })
...
FHIR.oauth2.ready().then(function(client) {
    ...
    client.request("Observation?" + query, { pageLimit: 0, flat: true})
        ...
        function(ob) {
            ...
            cholesterol_obs_data['title'] = 'No observations found';
            ...
            for (var index = 0; index < cholesterol.length; index++)
            {
                ...
                var data_units = cholesterol[index].valueQuantity.unit;
                var obs_time = cholesterol[index].effectiveDateTime;
                cholesterol_obs_data['data'].push({'value': obs_value, 
                                                    'units': data_units, 
                                                    'time': obs_time});
                if (index == 0)
                {
                  cholesterol_obs_data['title'] = cholesterol[index].code.text 
                                                  + " (" +  data_units 
                                                  + ") over time";
                }                         
            }
